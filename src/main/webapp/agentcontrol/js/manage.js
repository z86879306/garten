...
(function (window, document) {
        //全局省市区
    if(sessionStorage.agentToken){
        var token = sessionStorage.agentToken;
        console.log(token);
    }
    ...
    var address3 = {
            ...
            current_province: sessionStorage.province,
            current_city: sessionStorage.city === '' ? '城市' : sessionStorage.city,
            current_counties: sessionStorage.counties === '' ? '县区' : sessionStorage.counties,
            province: null,
            city: null,
            counties: null,
            grade: '未选择',
            province_isWritable: sessionStorage.province !== '',
            city_isWritable: sessionStorage.city !== '',
            counties_isWritable: sessionStorage.counties !== ''
        },
        kindergartens = {
            data:[],
            name: '幼儿园',
    ...
    Vue.component('public-address3',{
        ...
        '<div class="address3" @change="select_defaultValue($event)">' +
            '<select :disabled="disable || province_isWritable"  name="province" v-model="current_province">' +
                '<option v-for="(value,province,index) in data" :key="index" >{{province}}</option>' +
            '</select>' +
            '<select :disabled="disable || city_isWritable" v-show="current_province !== '+"'省份'"+'" name="city" v-model="current_city">' +
                '<option>城市</option>' +
                '<option v-for="(value,city) in data[current_province]">{{city}}</option>'+
            '</select>' +
            '<select :disabled="disable || counties_isWritable"  name="counties" v-model="current_counties" v-show="current_city !== '+"'城市'"+'">' +
                '<option>县区</option>' +
        ...
        watch:{
            current_province: function () {
                // this.current_province = this.current_province === ''? '省份' : this.current_province;
    ...
    var agentList = Vue.component('agent-list',{
       template:
           '<div class="agentList">' +
                '<h3 style="line-height: 150%;color: #006105;">代理商一览表: </h3>'+
                ...
                '<div class="look-Detaildata" v-if="show_detail">'+
                    ...
                    '<div>加盟起始时间: <input :value="agent_detail.agentStartTime" disabled type="text"></div>'+
                    '<div>加盟结束时间: <input :value="agent_detail.agentEndTime" disabled type="text"></div>'+
...
,
                    ...
                    success: function (data) {
                        ...
                        self.responseData.forEach(function (item,index) {
                            ...
                            agent.agentArea = item.province + item.city + item.countries;
                    ...
                    case '查看':
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                data.info.forEach(function (item,index) {
                                    var garten = {};
                                    garten.name = item.gartenName;
                                    garten.monitorCount = item.monitorCount;
                                    garten.attendanceCount = item.attendanceCount;
                                    tableItem[index] = garten;
                                });
                               self.agent_kindegartens.tableItem = tableItem;
                            }
                        });
                ...
                return {
                    ...
                    success: function (data) {
                        ...
                        data.info.list.forEach(function(item,index){
                            var kindergarten = {};
                            kindergarten.name = item.gartenName;
                            kindergarten.state = item.applicationState === 0 ? '未通过' : '通过'
    ...
    var kindergartenApply = Vue.component('kindergarten-apply',{
        ...
        '<div class="kindergartenApply">' +
            ...
            '<div v-show="show_current==='+"'开园申请列表'"+'">' +
                ...
                '<div class="look-Detaildata" v-if="show_detail">'+
                    ...
                    '<div>人数: <input :value="apply_detail.count" type="text" disabled></div>'+
                    '<div>代理费: <input :value="apply_detail.money1" type="text" disabled></div>'+
                    '<div>使用设备: <input :value="apply_detail.equipment" style="width:300px" type="text" disabled></div>'+
            ...
            '<div class="add-Newdata" v-show="show_current==='+"'开园申请'"+'">' +
                ...
                '<div>*签约人: <input v-model="applyData.name" type="text" ></div>'+
                '<div>*签约人联系方式: <input v-model="applyData.phoneNumber" type="text" ></div>'+
                '<div>*合同编号: <input v-model="applyData.contractNumber" type="text" ></div>'+
                '<p class="mark">注: 开园申请审核通过后需在 幼儿园管理板块→修改 填写合同起止日期</p>'+
                // '<div>组织编码索引: <input  type="text" ></div>'+
                '<div>*省市区(填写完整): <public-address3></public-address3></div>'+
                // '<div>详细地址: <input v-model="applyData." type="text" ></div>'+
                // '<div>幼儿园收费标准: <input v-model="applyData." type="text" ></div>'+
                '<div>*人数: <input v-model="applyData.count" type="text" ></div>'+
                '<div>*代理费: <input v-model="applyData.money" type="text" ></div>'+
                '<div>*使用设备: <input v-model="applyData.equipment" style="width:300px" type="text" ></div>'+
        ...
        data:function () {
            return {
                ...
                apply_detail: null,
                applyList: {
                    tableTitle: ['幼儿园名称', '合同编号', '人数', '使用设备', '代理费', '申请状态', '隐藏', '操作'],
                    tableItem: [
                        {
            ...
            xhr: function () {
                ...
                return {
                    ...
                    success: function (data) {
                        ...
                        self.responseData.forEach(function (item,index) {
                           var kindergarten = {};
                            kindergarten.kindergartenName = item.gartenName ;
                            kindergarten.pactNumber = item.contractNumber ;
                            kindergarten.peopleNumber = item.count ;
                            kindergarten.equipment = item.equipment ;
                            kindergarten.agentFee = item.money1;
                            kindergarten.applyState = '待审核';
                            kindergarten.operation = {
                                action:['查看','取消申请']
                            }
                            tableItem[index] = kindergarten;
                        });
                        self.applyList.tableItem = tableItem;
                    },
                }
            }
        },
    ...
    var kindergartenFollow = Vue.component('kindergarten-follow',{
        template:
            '<div class="kindergartenFollow">' +
                ...
                '</div>'+
                '<div class="follow" v-show="show_current==='+"'访园跟进'"+'">' +
                    '<div class="filter">' +
                        '<public-kindergartens></public-kindergartens>'+
                    '</div>'+
                    '<div class="kindergartenFollow-info">' +
                        '<div><span>主题</span><input v-model="visitData.title" type="text" class="kindergartenFollow-info-title" placeholder="标题"></div>'+
                        '<div class="kindergartenFollow-info-main"><span>正文</span><textarea v-model="visitData.content" placeholder="访园跟进信息"></textarea></div>'+
    ...
    var header = new Vue({
        el: '#header',
        data: {
            name: sessionStorage.agentName + '管理后台'
        }
    });
})(window, document);
